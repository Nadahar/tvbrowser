package org.tvbrowser.android.widgets;
...
import org.tvbrowser.android.data.ChannelDayProgram;
...
public class TVBrowserGridView extends View implements View.OnCreateContextMenuListener, View.OnClickListener {
  ...
  /**
   * Indicates we just received the touch event and we are waiting to see if it
   * is a tap or a scroll gesture.
   */
  ...
  private ArrayList<ChannelDayProgram> mDayPrograms;
  ...
  private ChannelDayProgram mSelectedChannel;
  public Program mSelectedBroadcast;
  ...
  private void initialize(Resources resources) {
    ...
    mVisibleStartX = TVBrowserGridView.getX(Calendar.getInstance());
    mNeedRepaint = true;
  ...
  public void setDayPrograms(ArrayList<ChannelDayProgram> dayPrograms) {
  ...
  private void remeasure(int width, int height) {
    ...
    int bitmapHeight = (mDayPrograms != null ? mDayPrograms.size() * TEXTHEIGHT + 2 * mDayPrograms.size()
    ...
    mBitmap = Bitmap.createBitmap(drawConfiguration.bitmapWidth, drawConfiguration.bitmapHeight, Bitmap.Config.RGB_565);
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (mNeedRepaint && mCanvas != null) {
      ...
      if (mDayPrograms != null) {
        ...
        int nowx = TVBrowserGridView.getX(now);
        for (ChannelDayProgram channel : mDayPrograms) {
          channel.drawChannelNames(mChannelNameCanvas, drawConfiguration, y);
          channel.drawBroadcasts(mCanvas, drawConfiguration, y, mVisibleStartX / MINUTE_WIDTH, visibleEndX,
              nowInMillis, nowx, mVisibleStartX, mSelectedBroadcast);
  ...
  public void doShowPress(MotionEvent motionEvent) {
    ...
    if (mSelectedChannel != null && mSelectedBroadcast != null) {
      Log.d(TVBrowser.LOGTAG, "doShowPress: " + mSelectedChannel.getChannelName() + ", " + mSelectedBroadcast.getTitle());
    }
    mNeedRepaint = true;
    invalidate();
  }
  ...
  private ChannelDayProgram getChannelByAbsolutePosition(int y) {
    y = y - TEXTHEIGHT - 2 * HORIZONTAL_GAP;
    if (y > 0) {
      y += mVisibleStartY;
      int channelId = (y / (TEXTHEIGHT + 2 * HORIZONTAL_GAP));
      if (channelId >= 0 && mDayPrograms.size() > channelId) {
        return mDayPrograms.get(channelId);
      }
    }
    return null;
  }

  private Program getBroadcastByAbsolutePosition(ChannelDayProgram channel, int x) {
    if (channel != null) {
  ...
  public void setVisiblePosition(int hour, int minute) {
    mVisibleStartX = TVBrowserGridView.getX(hour, minute) - drawConfiguration.bitmapWidth / 2;
    checkForValidXPosition();
    mNeedRepaint = true;
    invalidate();
  }
  ...
  public void onCreateContextMenu(ContextMenu contextMenu, View view, ContextMenu.ContextMenuInfo contextMenuInfo) {
    Program.createProgramContextMenu(mSelectedBroadcast, contextMenu);
  }
  ...
  private static int getX(int hour, int minute) {
    return (hour * 60 + minute) * MINUTE_WIDTH;
  }

  private static int getX(Calendar now) {
    return TVBrowserGridView.getX(now.get(Calendar.HOUR_OF_DAY), now.get(Calendar.MINUTE));
  }
