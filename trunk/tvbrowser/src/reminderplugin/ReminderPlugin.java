 ...
 */ public class ReminderPlugin extends Plugin implements ReminderTimerListener {
	...
	return new PluginInfo(name, desc, author, new Version(1, 0));
  		...
  		if (in!= null ) {
  			try {
				...
				String msg = mLocalizer.msg( "error.3" ,"Error loading reminder list!\n({0})" , exc);
				ErrorHandler.handle(msg, exc);
	  		}

	  		reminderList.removeExpiredItems();
	 		reminderList.setReminderTimerListener( this ); // mark the programs
	 		Iterator iter = reminderList.getReminderItems(); while (iter.hasNext()) {
  ...
  public void execute(devplugin.Program[] program) {
  	...
  	if (program.length==1) {
		...
		ReminderDialog dlg = new ReminderDialog(parent,program[0]);
...
util.io.Profiler.getDefault().show( "3.3" );
		if (dlg.getOkPressed()) {
			...
			if (reminderList== null ) {
	 		...
	 		reminderList.add( new ReminderListItem(program[0], dlg.getReminderMinutes()));
		} else {
		}	
		dlg.dispose();
  	} else {
