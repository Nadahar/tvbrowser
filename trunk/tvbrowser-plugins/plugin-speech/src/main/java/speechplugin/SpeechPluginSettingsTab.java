...
public final class SpeechPluginSettingsTab implements SettingsTab {
  ...
  public SpeechPluginSettingsTab(final SpeechPluginSettings settings) {
    mSettings = settings;
  ...
  public JPanel createSettingsPanel() {
    final PanelBuilder pb = new PanelBuilder(new FormLayout(
    ...
    final int engine = mSettings.getEngine();
    ...
    final PanelBuilder detailsBuilder = new PanelBuilder(new FormLayout(
    ...
    final ItemListener execListener = new ItemListener() {
    ...
    final List<String> voices = SpeechPlugin.getInstance().getAvailableVoices();
    ...
    pb.addSeparator(mLocalizer.msg("formattings", "Formattings"), cc.xyw(1, 8,
        2));
    mConfigPanel = new PluginProgramConfigurationPanel(SpeechPlugin
        .getInstance().getSelectedPluginProgramFormattings(), SpeechPlugin
        .getInstance().getAvailableLocalPluginProgramFormattings(),
  ...
  protected void showFileSettingsDialog() {
    ...
    final Window wnd = UiUtilities.getLastModalChildOf(MainFrame.getInstance());
  ...
  public void saveSettings() {
    final Object voice = mVoicesList.getSelectedValue();
    if (voice != null) {
    ...
    SpeechPlugin.getInstance().setAvailableLocalPluginProgramFormattings(
  ...
  private void showAvailableVoices() {
    final Cursor cursor = mPanel.getCursor();
    ...
    if (voice != null) {
      final int index = voices.indexOf(voice);
      if (index >= 0) {
