...
public class IDontWant2SeeListEntry {
  ...
  protected IDontWant2SeeListEntry(ObjectInputStream in, int version) throws IOException {
  ...
  protected boolean matches(Program p) {
    if(mPreSearchPart == null) {
      return mCaseSensitive ? p.getTitle().equals(mSearchText) : p.getTitle().equalsIgnoreCase(mSearchText);
    }
  ...
  protected void writeData(ObjectOutputStream out) throws IOException {
    // version 3, keep syncron with data file version from IDontWant2See.java
