...
import tvbrowser.core.plugin.ProgramConfigurationManager;
import tvbrowser.ui.mainframe.MainFrame;
import util.program.ProgramConfiguration;
import util.ui.Localizer;
import util.ui.ProgramConfigurationSettingsDialog;
 ...
 * @author René Mach
 */
public class ProgramConfigurationsSettings implements SettingsTab, ActionListener {
  private static Localizer mLocalizer = Localizer.getLocalizerFor(ProgramConfigurationsSettings.class);
  ...
  public JPanel createSettingsPanel() {
    try {
      ...
      mConfigurations = new SortableItemList("",ProgramConfigurationManager.getInstance().getAvailableProgramConfigurations());
      ...
      mConfigurations.getList().addMouseListener(new MouseAdapter() {
        public void mouseClicked(MouseEvent e) {
          if(SwingUtilities.isLeftMouseButton(e) && e.getClickCount() >= 2) {
            ProgramConfigurationSettingsDialog.createInstance(UiUtilities.getLastModalChildOf(MainFrame.getInstance()), (ProgramConfiguration)mConfigurations.getList().getSelectedValue(), ProgramConfigurationManager.getDefaultConfiguration(), true, true);
  ...
  public void saveSettings() {
    ...
    if(o != null && o.length > 0) {
      ProgramConfiguration[] p = new ProgramConfiguration[o.length];
    
      for(int i = 0; i < o.length; i++)
        p[i] = (ProgramConfiguration)o[i];
    
      ProgramConfigurationManager.getInstance().setAvailableProgramConfigurations(p);
    }
    else
      ProgramConfigurationManager.getInstance().setAvailableProgramConfigurations(new ProgramConfiguration[] {ProgramConfigurationManager.getDefaultConfiguration()});
    
    ProgramConfigurationManager.getInstance().store();
  }

  public void actionPerformed(ActionEvent e) {
    if(e.getSource() == mAdd)
      mConfigurations.addElement(ProgramConfigurationManager.getDefaultConfiguration());
