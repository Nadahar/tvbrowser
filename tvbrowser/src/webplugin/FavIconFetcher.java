...
import webplugin.aclib.image.ico.ICOFile;
...
public class FavIconFetcher {
   ...
   *  
   * @param urlString Load FavIcon for this URL
  ...
  public String fetchFavIconForUrl(String urlString) {
      ...
      try {
        ...
        if (img != null) {
          StringBuilder filename = new StringBuilder(Plugin.getPluginManager().getTvBrowserSettings().getTvBrowserUserHome());
          filename.append(File.separator).append("WebFavIcons").append(File.separator).append(urlString.substring(pos+2)).append(".png");
  ...
  private Image getBestIcon(ICOFile ico, int x, int y) {
    ...
    for (BitmapDescriptor desc : ico.getDescriptors()) {
  ...
  /**
   * @param img Create RenderImage for this Image
