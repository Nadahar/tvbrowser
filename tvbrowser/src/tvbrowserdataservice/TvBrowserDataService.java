...
public class TvBrowserDataService extends AbstractTvDataService {
    ...
    private static java.util.logging.Logger mLog = java.util.logging.Logger.getLogger(TvBrowserDataService.class.getName());

    /** The localizer for this class. */
    public static final util.ui.Localizer mLocalizer = util.ui.Localizer.getLocalizerFor(TvBrowserDataService.class);
    ...
    private static final String[][] DEFAULT_CHANNEL_GROUP_MIRRORS = new String[][] {
            new String[] { "http://www.tvbrowser.org/mirrorlists" }, new String[] { "http://www.tvbrowser.org/mirrorlists" },
            new String[] { "http://www.tvbrowser.org/mirrorlists" }, new String[] { "http://www.tvbrowser.org/mirrorlists" },
            new String[] { "http://www.tvbrowser.org/mirrorlists" }, new String[] { "http://www.tvbrowser.org/mirrorlists" }, };

    private static final String[] DEFAULT_CHANNEL_GROUP_NAMES = new String[] { "digital", "austria", "main", "local", "others", "radio" };

    private HashSet mChannelGroupSet, mDefaultChannelGroupSet;
    ...
    public static TvBrowserDataService getInstance() {
        if (mInstance == null) { throw new RuntimeException("no instance of TvBrowserDataService class available"); }
        return mInstance;
    }

    public void setWorkingDirectory(File dataDir) {
        mDataDir = dataDir;
        if (mChannelGroupSet == null) { return; }
    ...
    private ChannelGroup getChannelGroupById(String id) {
        ...
        while (it.hasNext()) {
            ...
            if (group.getId().equalsIgnoreCase(id)) { return group; }
        }
        return null;
    }
            ...
            for (int day = 0; day < dateCount; day++) {
                for (int levelIdx = 0; levelIdx < mSubscribedLevelArr.length; levelIdx++) {
                    ...
                    while (it.hasNext()) {
                        ...
                        addDownloadJob(dataBase, group.getMirror(), date, level, ch, ch.getCountry(), receiveDH, updateDH, group
                                .getSummary());
                    }
                }
                date = date.addDays(1);
    ...
    private void addDownloadJob(TvDataUpdateManager dataBase, Mirror mirror, Date date, String level, Channel channel, String country,
            DayProgramReceiveDH receiveDH, DayProgramUpdateDH updateDH, SummaryFile summary) throws TvBrowserException {
        // NOTE: summary is null when getting failed
        String completeFileName = DayProgramFile.getProgramFileName(date, country, channel.getId(), level);
        ...
        if (dataBase.isDayProgramAvailable(date, channel) && completeFile.exists()) {
            ...
            try {
                ...
                // TODO: dont' throw an exception; try to download the file
                // again
                throw new TvBrowserException(getClass(), "error.5", "Reading version of TV data file failed: {0}", completeFile
                        .getAbsolutePath(), exc);
            }
            ...
            if (summary != null) {
                int mirrorVersion = summary.getDayProgramVersion(date, country, channel.getId(), levelIdx);
                needsUpdate = (mirrorVersion > localVersion);
            }

            if (needsUpdate) {
                // We need an update -> Add an update job
                String updateFileName = DayProgramFile.getProgramFileName(date, country, channel.getId(), level, localVersion);
    ...
    Channel getChannel(String country, String channelName) {
        ...
        for (int i = 0; i < channelArr.length; i++) {
            ...
            if (channel.getCountry().equals(country) && channel.getId().equals(channelName)) { return channel; }
        }
    ...
    /**
     * If a Default-Group-Setting was created this will create it, otherwise it will 
     * return the Default-Hard-Coded groups as Fall-Back
     * 
     * @return Default ChannelGroup
     */
    public ChannelGroup[] getDefaultGroups() {
        if (DEFAULT_CHANNEL_GROUP_MIRRORS.length != DEFAULT_CHANNEL_GROUP_NAMES.length) { throw new RuntimeException(
                "invalid group names or group mirrors"); }

        if (mDefaultChannelGroupSet != null) {
            ChannelGroup[] grp = new ChannelGroup[mDefaultChannelGroupSet.size()];
            
            Iterator it = mDefaultChannelGroupSet.iterator();
            int i = 0;
            while (it.hasNext()) {
                grp[i] = (ChannelGroup) it.next();
                i++;
            }
            return grp;
        }
     ...
     * to store your dataservices settings to the file system.
     */
    ...
    public Channel[] getAvailableChannels() {
         ...
         * from a previous tvbrowser version. If the file exists, we ask the
         * user to load the new channel files from the internet.
         */
        ...
        if (channelList.size() == 0) {
            ...
            if (file.exists()) {
                ...
                Object[] options = { "Jetzt herunterladen (empfohlen)", "Sp\u00E4ter neu einrichten" };
                int n = JOptionPane.showOptionDialog(null, "Es befindet sich keine aktuelle Senderliste auf dem System.\n"
                        ...
                        "Keine Senderlisten verf\u00FCgbar", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, options,
                        options[0]);
                ...
                if (n == 0) {
                    ...
                    if (mChannelsTempArr == null) { return new Channel[0]; }
  ...
  public Channel[] checkForAvailableChannels(ProgressMonitor monitor) throws TvBrowserException {
    ...
    monitor.setMessage("Checking for new ChannelGroups");

    try {
      checkForNewGroups();
    }catch(IOException e) {
      throw new TvBrowserException(TvBrowserDataService.class, "checkForNewGroups.failed","Checking for new groups failed.");
    }
    ...
    while (it.hasNext()) {
      i++;
      ChannelGroup group = (ChannelGroup) it.next();
      monitor.setMessage(mLocalizer.msg("checkingForAvailableChannels", "Checking group {0} - ({1} of {2})", group.getName(), "" + i,
          "" + mChannelGroupSet.size()));
    ...
    /**
     * Check for new Groups and store them into the active Grouplist.
     * The new Group is also added to the Default-Groups-List. If the User presses
     * "Reset" the new Group will still be there...
     * 
     * If a group was removed, it will be removed in the Default and the User-Group
     */
    private void checkForNewGroups() throws IOException {
        ...
        if (groups != null) {
            ...
            ChannelGroup[] def = getDefaultGroups();
            
            ArrayList defgrp = new ArrayList();
            
            for (int i = 0; i < def.length; i++) {
                defgrp.add(def[i]);
            }
            
            // Create a copy of the ChannelGroup
            ArrayList copy = new ArrayList(groups);
            
            // Run through all Default-Groups
            for (int i=0;i<def.length;i++) {
                
                boolean found = false;
                
                // Run through all new Groups
                for (int j=0;j<groups.size();j++) {
                    ChannelGroup g = (ChannelGroup)groups.get(j);
                    
                    // Group was found, remove it
                    if (g.getId().equals(def[i].getId())) {
                        found = true;
                        copy.remove(g);
                    }
                }
                
                // Doesn't exist anymore, delete it in the Default-List and the User-List
                if (!found) {
                    defgrp.remove(def[i]);
                    
                    // Try to find Item in the User-Group
                    ChannelGroup uGroup = null;
                    
                    Iterator it = mChannelGroupSet.iterator();
                    while (it.hasNext()) {
                        ChannelGroup g = (ChannelGroup)it.next();
                        if (g.getId().equals(def[i].getId())) {
                            uGroup = g;
                        }
                    }
                    
                    // Delete Group if found
                    if (uGroup != null) {
                        mChannelGroupSet.remove(uGroup);
                    }
                }
            }
            
            // Copy new groups into the defgrp
            for (int i =0; i < copy.size();i++) {
                defgrp.add(copy.get(i));
                mChannelGroupSet.add(copy.get(i));
                ((TvBrowserDataServiceSettingsPanel)getSettingsPanel()).refreshGroupList();
            }
            
            // Store defgrp as Default-Group
            mDefaultChannelGroupSet = new HashSet();
            mDefaultChannelGroupSet.addAll(defgrp);
            saveDefaultChannelGroup();
        }
    }
    
    /**
     * Loads the Defaul ChannelGroup from the Settings.
     * If the Setting doesn't exist, it will be null
     */
    private void loadDefaultChannelGroup() {
        String groupNames =  mSettings.getProperty("defaultgroupname");
        
        if (groupNames == null) {
            mDefaultChannelGroupSet = null;
            return;
        }
    ...
    /**
     * Stores the DefaultChannelGroup 
     */
    private void saveDefaultChannelGroup() {
        Iterator it = mDefaultChannelGroupSet.iterator();
        
        String grplist = new String();
        
        while (it.hasNext()) {
            grplist += ((ChannelGroup)it.next()).getId() + ":";
        }
        ...
        mSettings.setProperty("defaultgroupname", grplist);
    ...
    public PluginInfo getInfo() {
        return new devplugin.PluginInfo("TV-Browser", mLocalizer.msg("description", "Die eigenen TV-Daten des TV-Browser-Projektes"),
                "Til Schneider, www.murfman.de", new Version(0, 2), mLocalizer.msg("license", ""));
    }
