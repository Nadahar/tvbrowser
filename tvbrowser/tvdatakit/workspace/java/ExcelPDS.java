...
public class ExcelPDS extends AbstractPrimaryDataService {
      ...
      try {
        ...
        for (int row = 6; row <= sheet.getLastRowNum(); row++) {
          ...
          if (date != null) {
            ...
            ProgramFrame frame = extractProgramFrame(sheet, channelNr, row);
   ...
   */
  private ProgramFrame extractProgramFrame(HSSFSheet sheet, int sheetNr, int row) {
  ...
  private HSSFSheet[] loadAndCheckSheets() {
    ...
    // Check the sheets
    ArrayList validSheetList = new ArrayList();
  ...
  private String getCellString(HSSFSheet sheet, int col, int row) {
    ...
    if (xlsCell == null) {
      ...
      return xlsCell.getStringCellValue().trim();
    }
  }
